...
class MotionLibBase():
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        if self._sampling_prob[self._curr_motion_ids].sum() > 0:
            ...
            self._sampling_batch_prob = torch.ones(len(self._curr_motion_ids)).to(self._device) / len(self._curr_motion_ids)
        ...
        for f in tqdm(range(len(res_acc))):
            ...
            if flags.real_traj:
                q_gts.append(curr_motion.quest_motion['quest_trans'])
                q_grs.append(curr_motion.quest_motion['quest_rot'])
                q_gavs.append(curr_motion.quest_motion['global_angular_vel'])
                q_gvs.append(curr_motion.quest_motion['linear_vel'])
            del curr_motion
            
        self._motion_lengths = torch.tensor(_motion_lengths, device=self._device, dtype=torch.float32)
        self._motion_fps = torch.tensor(np.array(_motion_fps), device=self._device, dtype=torch.float32)
        ...
        if flags.real_traj:
            self.q_gts = torch.cat(q_gts, dim=0).float().to(self._device)
            self.q_grs = torch.cat(q_grs, dim=0).float().to(self._device)
            self.q_gavs = torch.cat(q_gavs, dim=0).float().to(self._device)
            self.q_gvs = torch.cat(q_gvs, dim=0).float().to(self._device)
