...
if __name__ == "__main__":
    ...
    for i in range(num_envs):
        ...
        actor_handle = gym.create_actor(env, asset, pose, "actor", 0, 0, 0) # col_group, col_filter, 0
    ...
    while not gym.query_viewer_has_closed(viewer):
        ...
        # pd_tar = torch.zeros((1, 153)).to(device)
        # pd_tar[0, 55] = np.pi
        # pd_tar = torch.from_numpy(sRot.from_rotvec(dof_pos.reshape(-1, 3)).as_euler("XYZ")).float().to(device).reshape((1, -1))
        pd_tar = dof_pos
