...
if __name__ == "__main__":
    ...
    for i in range(num_envs):
        ...
        actor_handle = gym.create_actor(env, asset, pose, "actor", 0, 1, 0) # col_group, col_filter, 0
        ...
        print(camera_handle);print(camera_handle);print(camera_handle);print(camera_handle);print(camera_handle);print(camera_handle);
    ...
    while True:
        ...
        # gym.clear_lines(viewer)
        # for idx in range(rb_pos.shape[1]):
        #     color_inner = (0.3, 0.3, 0.3)
        #     sphere_geom_marker = gymutil.WireframeSphereGeometry(0.01, 4, 4, None, color=color_inner)
        #     sphere_pose = gymapi.Transform(gymapi.Vec3(rb_pos[0, idx, 0], rb_pos[0, idx, 1], rb_pos[0, idx, 2]), r=None)
        #     gymutil.draw_lines(sphere_geom_marker, gym, viewer, envs[0], sphere_pose) 
        ...
        for env_ptr in envs:
            camera_rgba_tensor = gym.get_camera_image_gpu_tensor(sim, env_ptr, camera_handle, gymapi.IMAGE_COLOR)
            rgba_image = gymtorch.wrap_tensor(camera_rgba_tensor)
